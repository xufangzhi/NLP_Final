...
def load_model():
    ...
    lm_model = torch.load("/data/linqika/xufangzhi/SQUAD/train_lm/checkpoints/physics+tqa+squad_spanmask_2.pth")
...
def get_data(split, tokenizer, max_len):
    ...
    with open("jsons/squad.json", "r", encoding="utf-8", errors="surrogatepass") as file:
...
def train(model, tokenizer, process_data_train, process_data_dev, dev_qids_list, optimizer, scheduler, epochs, batch_size, device, save_model):
    for epoch_i in range(0, epochs):
        ...
        if save_model:
            torch.save(model, "checkpoints/without_postpt_e"+str(epoch_i+1)+".pth")
        
        res_json = dev(model, tokenizer, process_data_dev, dev_qids_list, device)
        with open("res_final_e"+str(epoch_i+1)+".json",'w') as file:
...
def main(argv):
    ...
    #model.roberta = torch.load("/data/linqika/xufangzhi/SQUAD/train_lm/checkpoints/physics+tqa+squad_spanmask_2.pth").roberta
